...
def main(*args):
    try:
        ...
        if "您需要先登录才能继续本操作"  in c:
            pusher('52pojie账户' +str(j)+ "Cookie过期", c)
            msg += "cookie_52pj失效，需重新获取"
        elif "恭喜"  in c:
            msg += "52pj签到成功"
            pusher("52pj账户" +str(j)+ "签到成功", c)
        elif "不是进行中的任务" in c:
            msg += "不是进行中的任务"
        else:
            print(c)
        ...
        pusher("52pojie账户" +str(j)+ "访问出错",msg)
    return msg + "\n"
...
def pjRate(*args):
    try:
        ...
        for tid in tidlist:
            ...
            if "需要登录" in r.text:
                pusher("52pojie账户" +str(j)+ "Cookie过期")
                msg += "cookie_52pj失效，需重新获取"
                break
            ...
            if "succeedhandle_rate" in r.text:
                ...
                pusher("52pojie账户" +str(j)+ "免费评分详情",msg)
                break
            elif "评分数超过限制" in r.text:
                msg += re.findall("errorhandle_rate\('.*'", r.text)[0][18:-1]
                pusher('52pojie账户 ' +str(j)+ '评分数超过限制',msg)
                break
            else:
                msg += re.findall("errorhandle_rate\('.*'", r.text)[0][18:-1]
                msg += "\n"
                pusher("52pojie账户" +str(j) ,msg)
    except:
        pusher("52pojie账户" +str(j)+ "免费评分失败")
        pass
    return msg + "\n"
